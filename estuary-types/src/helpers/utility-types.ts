...
>(
    schema: S,
  ): S => (schema as unknown as O).deepPartial() as unknown as S;

/**
 * (WIP) Returns a schema with only the passed keys required
 * Loses type information
 */
export const schemaRequireOnly = <
  S extends z.ZodTypeAny,
  O extends z.AnyZodObject,
  K extends keyof S,
>(
 ...
 * WIP
 */
export const schemaOmit = <S extends z.ZodTypeAny>(
  schema: S,
  keys: string[],
) => {
  const keyObj = keys.reduce(
